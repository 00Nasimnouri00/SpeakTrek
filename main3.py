...
def analyze_emotion_and_pitch(segment_pitches, message):
    ...
    for segment_data in segment_pitches:
        ...
        # Send the text and pitch values to the user
        bot.reply_to(message, f"Text: {text}\nPitch Values: {segment_pitch_values}")

        # Get emotion analysis for the text
        ...
        if hasattr(response, 'status_code') and response.status_code == 200:
            ...
            bot.reply_to(message, f"Emotion Analysis:\n{text_content}")
...
def handle_audio(message):
    try:
        ...
        for j in range(len(emotion_l)):
            ...
            segment_score = calculate_total_score(Final_res['choices'][0]['message']['content'])
